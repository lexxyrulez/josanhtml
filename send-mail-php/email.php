...
function sendMail($name, $email, $phone, $message, $subject, $date)
{
  ...
  $mail->Body = "<b>Name:</b> {$name}<br><b>Phone:</b> {$phone}<br><b>Email:</b> {$email}<br><br><b>Message:</b><br><br>
...
function start()
{
  if (
    ...
    sendMail($name, $email, $phone, $message, $subject, $date);
  } else {
    $_SESSION["mail_error"] = true;
    redirectToIndex();
  }
